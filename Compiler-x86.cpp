...
std::ifstream ReadWindresLog("WindresLog.txt");
...
int main(int argc, char* argv[]) {
    ...
    string Path = GetPath() + "\\Compiler-x86.exe";
    // cout << "Path: " << Path << endl;
    
    // if (argc == 2 && (strcmp(argv[1], "Compiler-x86.exe") == 0 || strcmp(argv[1], Path.c_str()) == 0 || strcmp(argv[1], "compiler-x86.exe") == 0)) {
    //     NoOptions();
    //     return EXIT_FAILURE;
    // }
    ...
    if (argc == 1 && strcmp(argv[0], "Compiler-x86.exe") == 0){
        NoOptions();
        return 0;
    }
    ...
    if (NeedHelp){
        std::cout << "这个程序支持的参数有: -f -s -o -i -w -n -l -h -c -a -r -e -d" << std::endl;
        ...
        cout << "那么剩下的都不需要参数, -w = -Wall, -l = -leasyx, -n = -lwinmm, -a = -lshlwapi, -h = -mwindows, -c = -fsyntax-only -e = -lole32 -d = -lgdi32\n";
    ...
    while ((option = getopt(argc, argv, "f:s:o:lhcr:nai:we")) != -1) {
        switch (option) {
            case 'f':{
                // 将输入的文件名 拼成编译脚本
                CompileFile = optarg;
